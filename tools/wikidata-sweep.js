...
/* Under this at sweep time is not a card. See the screen in main() for why a
   MISSING birth date is not a rejection, and what that leaves uncovered. */
const MIN_AGE = 18;
const ageYears = iso => {
  const t = Date.parse(iso);
  return Number.isFinite(t) ? (Date.now() - t) / (365.25 * 24 * 60 * 60 * 1000) : Infinity;
};
...
function classifyQuery(items) {
  return `
SELECT ?item ?performer ?corporate ?anglo ?dead ?india ?office ?party ?crime ?blockedOcc ?dob WHERE {
...
async function main() {
  ...
  const reasons = { unclassified: 0, notPerformer: 0, corporate: 0, notAnglo: 0, excluded: 0, minor: 0 };
  for (const [yt, v] of [...found]) {
    ...
    /* ── NO CHILDREN (2026-08-17) ────────────────────────────────────────────
       Added after an audit of the LIVE deck found 46 cards belonging to people
       Wikidata records as minors — including two of the nine RUBY cards, an
       11-year-old and a 12-year-old, which is to say the rarest cards in the
       game were children. Every screen above asks what someone IS; none of them
       asked how old they are, and nobody had noticed because the institution
       rule reads as though it covers "who gets a card" completely. It does not.

       A card carries a real face, a real name and numbers presented as combat
       stats. For an adult public figure that is the ordinary bargain of being
       one; for a fourteen-year-old it is a different thing entirely, and the
       opt-out does not repair it after the fact because a minor is not the
       person who would send that email.

       KNOWN-MINOR ONLY, DELIBERATELY. A missing P569 does not reject — most
       creators have no birth date recorded, and refusing everyone unverified
       would empty the sweep. So this is a floor, not a guarantee, and its
       blind spot is the one that matters most: a family or kids' channel is
       typed "YouTube channel", carries no birth date at all, and features
       children constantly. `tools/minors-audit.js --names` exists to give that
       blind spot a shape for a human to read; this screen cannot see it. */
    if (f.dob && ageYears(f.dob) < MIN_AGE) {
      reasons.minor++; found.delete(yt); continue;
    }
  }
  console.log(`\n  ${swept} swept -> ${found.size} kept`);
  console.log(`    dropped: ${reasons.notPerformer} not a performer · ${reasons.corporate} corporate veto · ` +
              `${reasons.notAnglo} non-anglophone · ${reasons.excluded} excluded claim · ${reasons.minor} known minor · ${reasons.unclassified} unclassified`);
  ...
  const lines = [
    ...
    '# Anyone Wikidata records (P569) as under 18 is refused. A MISSING birth date is',
    '# not a rejection, so this is a floor and not a guarantee — a family or kids',
    '# channel carries no birth date at all. See tools/minors-audit.js.',
