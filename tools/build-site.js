...
async function main() {
  ...
  /* THE SOCIAL PREVIEW IMAGE. index.html points og:image and twitter:image at
     /og.png, so without this line every share renders a broken picture — which
     is worse than the no-image state it replaced, because a scraper that fetches
     a 404 caches the failure.

     Warned about rather than made fatal: og.png is a screenshot of og.html (see
     the note at the top of that file), it is not generated by any build step,
     and a fresh clone that has not taken it yet must still be able to produce a
     working site. A deploy with no preview image is a worse launch, not a broken
     one — but it should never be a SILENT worse launch. */
  if (await exists(from('og.png'))) {
    await cp(from('og.png'), to('og.png'));
  } else {
    console.warn('WARNING: no og.png — every shared link will show a broken preview image.');
    console.warn('         Open og.html, screenshot the 1200x630 frame, save it as og.png.');
  }
